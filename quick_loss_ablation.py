...
class QuickLossAblationTrainer():
    def __init__(self, config, lambda_gate_entropy=0.01, lambda_gate_balance=0.02, lambda_gate_feedback=0.2):
        self.config = config
    ...
    def calculate_all_losses(self, contrast_pairs, contrast_pairs_r, contrast_pairs_f, contrast_pairs_m, contrast_pairs_m2, contrast_pairs_b1, contrast_pairs_b1_2, contrast_pairs_b1_ind, contrast_pairs_m_ind, cas_top, label, topk_indices, action_flow, action_rgb, cls_agnostic_gt, actionness1, actionness2, gate_weights, embedding_mixed, embedding_branch1, action_branch1, action_branch2):
        ...
        # 使用传入的门控权重参数，对比损失使用固定值
        cost = (base_loss + 
                class_agnostic_loss + 
                5*modality_consistent_loss + 
                0.1*adjusted_contrastive_loss +  # 对比损失使用固定值
    ...
    def train(self):
        ...
        for epoch in range(min(2, self.config.num_epochs)):  # 仅训练2个epoch以快速测试
            for i, (_data, _label, temp_anno, _, _) in enumerate(self.train_loader):
                ...
                if i % 10 == 0:  # 每10个batch打印一次
                    print(f"[GE:{self.lambda_gate_entropy}, GB:{self.lambda_gate_balance}, GF:{self.lambda_gate_feedback}] Epoch {epoch}, Batch {i}, Loss: {cost.item():.4f}")

                if i % 50 == 0 and i > 0:  # 每50个batch进行一次评估（快速测试）
                    with torch.no_grad():
                        self.net.eval()
                        ...
                        if mean_ap > self.best_mAP:
                            ...
                            model_filename = f"quick_ablation_ge_{self.lambda_gate_entropy}_gb_{self.lambda_gate_balance}_gf_{self.lambda_gate_feedback}.pkl"
                            torch.save(self.net.state_dict(), os.path.join(self.config.model_path, model_filename))

                        print(f"[GE:{self.lambda_gate_entropy}, GB:{self.lambda_gate_balance}, GF:{self.lambda_gate_feedback}] Epoch {epoch}, Batch {i}, mAP: {mean_ap*100:.2f}, Acc: {test_acc*100:.2f}, Best mAP: {self.best_mAP*100:.2f}")
                        
                        # 如果是快速测试，可以提前结束
                        if epoch == 1 and i > 100:  # 在第二个epoch的100个batch后停止
                            return


def run_quick_gate_loss_ablation(args):
    """运行快速门控损失函数消融实验"""
    
    # 定义门控相关权重的测试值: 0.01, 0.05, 0.1, 0.2
    gate_weights_values = [0.01, 0.05, 0.1, 0.2]
    
    print("开始运行快速门控损失函数消融实验...")
    print(f"门控相关权重测试值: {gate_weights_values}")
    print()
    ...
    for ge in gate_weights_values:
        for gb in gate_weights_values:
            for gf in gate_weights_values:
                combinations.append((ge, gb, gf))
    
    print(f"总共 {len(combinations)} 个门控权重组合:")
    for i, (ge, gb, gf) in enumerate(combinations):
        print(f"  组合 {i+1}: GE={ge}, GB={gb}, GF={gf}")
    print()
    
    # 限制测试数量以节省时间
    test_combinations = combinations[:8]  # 仅测试前8个组合
    print(f"实际测试 {len(test_combinations)} 个组合（为节省时间）:")
    for i, (ge, gb, gf) in enumerate(test_combinations):
        print(f"  组合 {i+1}: GE={ge}, GB={gb}, GF={gf}")
    print()
    
    for i, (lambda_gate_entropy, lambda_gate_balance, lambda_gate_feedback) in enumerate(test_combinations):
        print(f"正在运行门控权重组合 {i+1}/{len(test_combinations)}")
        print(f"  参数: GE={lambda_gate_entropy}, GB={lambda_gate_balance}, GF={lambda_gate_feedback}")
        
        # 修改模型保存路径以区分不同实验
        config.model_path = os.path.join(config.model_path, "quick_gate_ablation", f"ge_{lambda_gate_entropy}_gb_{lambda_gate_balance}_gf_{lambda_gate_feedback}")
        os.makedirs(config.model_path, exist_ok=True)
        
        trainer = QuickLossAblationTrainer(config, lambda_gate_entropy, lambda_gate_balance, lambda_gate_feedback)
        trainer.train()
        
        print(f"  门控权重组合 {i+1} 完成\n")


def main():
    args = parse_args()
    run_quick_gate_loss_ablation(args)
