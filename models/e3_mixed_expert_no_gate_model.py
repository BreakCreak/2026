...
class MixedExpert_Fixed(nn.Module):
...
class BaseModel_MixedExpert_NoGate(nn.Module):
    def __init__(self, len_feature, num_classes, config=None):
        super(BaseModel_MixedExpert_NoGate, self).__init__()
        ...
        # 混合专家模块 - 输出维度为512
        self.mixed_expert = MixedExpert_Fixed(512, 512)  # 输入512,512，输出512
    ...
    def forward(self, x, inference=False):
        ...
        # 不使用学习的门控，而是使用固定权重
        # 这里使用均匀权重 [0.5, 0.5] 作为固定权重
        batch_size, _, seq_len = gate_logits.shape
        gate_weights = torch.zeros(batch_size, 2, seq_len, device=gate_logits.device)
        gate_weights[:, 0, :] = 0.5  # Branch1权重
        gate_weights[:, 1, :] = 0.5  # Mixed分支权重
        
        # 固定权重，不参与梯度更新
        gate_weights = gate_weights.detach()

        branch1_w = gate_weights[:, 0:1, :]  # [B, 1, T]
        branch2_w = gate_weights[:, 1:2, :]  # [B, 1, T]
        ...
        return (
            ...
            embedding_mixed,
            embedding_branch1,
            gate_weights,  # 虽然是固定的，但为了接口兼容
            action_branch1,
            action_branch2
        )


class E3_MixedExpert_NoGate(nn.Module):
    def __init__(self, cfg):
        super(E3_MixedExpert_NoGate, self).__init__()
        self.len_feature = 2048
        self.num_classes = 20

        self.actionness_module = BaseModel_MixedExpert_NoGate(self.len_feature, self.num_classes, cfg)
