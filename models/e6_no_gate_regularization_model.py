...
class MixedExpert_NoReg(nn.Module):
...
class BaseModel_NoGateRegularization(nn.Module):
    def __init__(self, len_feature, num_classes, config=None):
        super(BaseModel_NoGateRegularization, self).__init__()
        ...
        # 混合专家模块 - 输出维度为512
        self.mixed_expert = MixedExpert_NoReg(512, 512)  # 输入512,512，输出512
...
class E6_NoGateRegularization(nn.Module):
    def __init__(self, cfg):
        super(E6_NoGateRegularization, self).__init__()
        self.len_feature = 2048
        self.num_classes = 20

        self.actionness_module = BaseModel_NoGateRegularization(self.len_feature, self.num_classes, cfg)
