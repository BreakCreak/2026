...
class BaseModel_RGB_Only(nn.Module):
    def __init__(self, len_feature, num_classes, config=None):
        super(BaseModel_RGB_Only, self).__init__()
    ...
    def forward(self, x, inference=False):
        ...
        # 仅使用RGB分支
        emb_branch1 = emb_rgb  # [B, 512, T]

        # 移除混合专家分支
        emb_mixed = emb_rgb  # 使用RGB作为替代

        # 移除门控机制 - 直接使用RGB分支
        gate_weights = torch.zeros(input.size(0), 2, input.size(2), device=input.device)
        gate_weights[:, 0, :] = 1.0  # 全部选择RGB分支

        # 仅RGB分支的动作性预测
        action_branch1 = torch.sigmoid(self.cls_rgb(emb_rgb)).squeeze(1)  # [B, T]
        action_branch2 = torch.sigmoid(self.cls_rgb(emb_rgb)).squeeze(1)  # [B, T] (复制作为占位符)

        # 仅使用RGB分支
        actionness_selected = action_branch1

        # 保存用于对比学习的嵌入
        embedding_flow = torch.zeros_like(emb_rgb.permute(0, 2, 1))  # 占位符
        embedding_rgb = emb_rgb.permute(0, 2, 1)
        embedding_mixed = emb_rgb.permute(0, 2, 1)  # 占位符
        ...
        action_flow = torch.zeros_like(action_rgb)  # 占位符

        # 最终的actionness2是RGB分支的结果
...
class E1_RGB_Only(nn.Module):
    def __init__(self, cfg):
        super(E1_RGB_Only, self).__init__()
        self.len_feature = 2048
        self.num_classes = 20

        self.actionness_module = BaseModel_RGB_Only(self.len_feature, self.num_classes, cfg)
    ...
    def forward(self, x):
        ...
        # Flow分支对比对使用占位符
        CAf = torch.zeros_like(CAr)
        CBf = torch.zeros_like(CBr)
        IAf = torch.zeros_like(IAr)
        IBf = torch.zeros_like(IBr)

        # Mixed branch contrastive learning (使用RGB分支作为占位符)
        CAm, CBm = self.consistency_snippets_mining1(
            aness_bin1, aness_bin2, actionness1, embedding_rgb, k_C
        )
        IAm, IBm = self.Inconsistency_snippets_mining1(
            aness_bin1, aness_bin2, actionness1, embedding_rgb, k_I
        )

        # Branch1 contrastive learning (RGB分支)
        ...
        # 为混合专家分支添加额外的对比学习 - 使用RGB作为占位符
        CAm2, CBm2 = self.consistency_snippets_mining1(
            aness_bin1, aness_bin2, actionness2, embedding_rgb, k_C
        )
        IAm2, IBm2 = self.Inconsistency_snippets_mining1(
            aness_bin1, aness_bin2, actionness2, embedding_rgb, k_I
        )
        ...
        # Branch2单独的对比对 (使用RGB作为占位符)
        CAm_ind, CBm_ind = self.consistency_snippets_mining1(
            action_branch2_bin, action_branch2_bin, action_branch2, embedding_rgb, k_C
        )
        Im_ind, Im_ind_ = self.Inconsistency_snippets_mining1(
            action_branch2_bin, action_branch2_bin, action_branch2, embedding_rgb, k_I
        )
