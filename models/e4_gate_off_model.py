...
class MixedExpert_GateOff(nn.Module):
...
class BaseModel_GateOff(nn.Module):
    def __init__(self, len_feature, num_classes, config=None):
        super(BaseModel_GateOff, self).__init__()
        ...
        # 混合专家模块 - 输出维度为512
        self.mixed_expert = MixedExpert_GateOff(512, 512)  # 输入512,512，输出512
    ...
    def forward(self, x, inference=False):
        ...
        # 门控输出固定（例如 uniform 分布或 detach / no grad）
        # 这里使用固定的均匀分布 [0.5, 0.5]，不参与梯度更新
...
class E4_GateOff(nn.Module):
    def __init__(self, cfg):
        super(E4_GateOff, self).__init__()
        self.len_feature = 2048
        self.num_classes = 20

        self.actionness_module = BaseModel_GateOff(self.len_feature, self.num_classes, cfg)
