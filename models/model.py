...
class MixedExpert(nn.Module):
    def __init__(self, c_in, c_out=1024):  # 输出维度改为1024，与RGB+Flow相加后维度一致
        super().__init__()
...
class BaseModel(nn.Module):
    def __init__(self, len_feature, num_classes, config=None):
        ...
        # 混合专家模块 - 输出维度为1024，与RGB+Flow相加后维度一致
        self.mixed_expert = MixedExpert(512, 512)  # 输入512,512，输出512 - 修改输入维度以匹配处理后的特征
        self.cls_mixed = nn.Conv1d(1024, 1, 1)  # 混合专家分支的分类头，输入1024
        ...
        self.gate_module = nn.Sequential(
            ...
            nn.Sigmoid()  # 使用Sigmoid确保输出在[0,1]范围内，接近0或1
        )
    ...
    def forward(self, x, inference=False):
        ...
        # RGB + Flow 相加 (维度为1024)
        emb_branch1 = emb_rgb + emb_flow  # [B, 1024, T]
        ...
        gate_weights = self.gate_module(combined_for_gating)  # [B, 2, T]
        
        # 为了实现接近0或1的选择，我们可以使用温度参数的Gumbel-Softmax或直接阈值
        # 这里我们使用阈值方式，让模型学会输出接近0或1的值
        if not self.training or inference:  # 推理时使用硬选择
            gate_weights_binary = torch.zeros_like(gate_weights)
            max_indices = torch.argmax(gate_weights, dim=1, keepdim=True)  # [B, 1, T]
            gate_weights_binary.scatter_(1, max_indices, 1.0)  # 设置最大值位置为1
            branch1_w, branch2_w = gate_weights_binary[:, 0:1, :], gate_weights_binary[:, 1:2, :]
        else:  # 训练时使用软选择，但鼓励接近0或1
        ...
        # 分支1: RGB + Flow 相加
        action_branch1 = action_flow+action_rgb  # [B, T]，使用对应的分类头
