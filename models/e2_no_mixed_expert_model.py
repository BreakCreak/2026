...
class BaseModel_No_MixedExpert(nn.Module):
    def __init__(self, len_feature, num_classes, config=None):
        super(BaseModel_No_MixedExpert, self).__init__()
        ...
        # 门控模块 - 现在只选择RGB+Flow分支
        self.gate_logits = nn.Sequential(
            nn.Conv1d(512, 512, 3, padding=1),  # 只有RGB+Flow分支 (512)
            nn.ReLU(),
            nn.Conv1d(512, 1, 1),  # 输出1个门控logit，因为只有一个分支
        )
    ...
    def forward(self, x, inference=False):
        ...
        # 移除混合专家分支 - 使用简单融合
        emb_mixed = emb_rgb + emb_flow  # 简单相加作为替代

        # 将Branch1特征用于门控（只有一个分支）
        # 我们保持原来的门控设计，但只关注RGB+Flow相加分支
        combined_for_gating = emb_branch1  # [B, 512, T]

        # 门控 - 由于没有混合专家，这里简化处理
        gate_logits_temp = self.gate_logits(combined_for_gating)  # [B, 1, T]
        # 为了保持与原结构兼容，我们扩展为2维
        gate_logits = torch.cat([gate_logits_temp, torch.zeros_like(gate_logits_temp)], dim=1)  # [B, 2, T]
        ...
        # 由于没有混合专家，我们主要依赖RGB+Flow相加分支
        if not self.training:
            # 推理时偏向使用Branch1
            gate_weights_hard = torch.zeros_like(gate_logits)
            gate_weights_hard[:, 0, :] = 1.0  # 偏向Branch1
            branch1_w, branch2_w = gate_weights_hard[:, 0:1, :], gate_weights_hard[:, 1:2, :]
        else:
            # 训练时也偏向Branch1，但允许学习
            branch1_w = torch.ones(input.size(0), 1, input.size(2), device=input.device) * 0.8
            branch2_w = torch.ones(input.size(0), 1, input.size(2), device=input.device) * 0.2

        # 分支1: RGB + Flow 相加（分别分类再加和）
        action_branch1 = torch.sigmoid(self.cls_rgb(emb_rgb)).squeeze(1) + torch.sigmoid(self.cls_flow(emb_flow)).squeeze(1)  # [B, T]

        # 分支2: 简单融合分支
        action_branch2 = torch.sigmoid(self.cls_rgb(emb_mixed)).squeeze(1)  # [B, T]
...
class E2_No_MixedExpert(nn.Module):
    def __init__(self, cfg):
        super(E2_No_MixedExpert, self).__init__()
        self.len_feature = 2048
        self.num_classes = 20

        self.actionness_module = BaseModel_No_MixedExpert(self.len_feature, self.num_classes, cfg)
    ...
    def forward(self, x):
        ...
        # Mixed branch contrastive learning (使用简单融合替代)
